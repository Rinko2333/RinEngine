...
- `SettingsManager` 已有 `language` 属性（默认 `"zh"`）和 NOTIFY 信号
- `SettingsScreen.qml` 已有语言选择 ComboBox（预留，当前 disabled）
- Qt 6.5 内置 `qsTr()` / `QT_TR_NOOP()` 国际化基础设施
- CMake 构建系统稳定，支持多 target、多 Qt 模块
- 所有 UI 文本目前硬编码为中文

### Phase 6 待做

1. **LocalizationManager (C++)** — 文本 ID → 多语言映射，运行时切换，信号通知 QML 刷新
2. **l10n/ 翻译字典** — zh.json / en.json / ja.json，覆盖所有 UI 文本和系统消息
3. **脚本多语言** — 脚本中 `say` 文本支持 ID 引用，通过 LocalizationManager 查找
4. **资源打包系统** — Qt `.qrc` 管理内置资源，外部资源包加载
5. **跨平台发布** — Windows/macOS/Linux 构建验证，一键导出工具

### Phase 6 注意事项

- `LocalizationManager` 必须注册为 QML 单例，emit `languageChanged` 时 QML 绑定自动重求值
- QML 中的文本绑定需要显式依赖 `LocalizationManager.tr("KEY")` 或使用属性绑定
- 脚本多语言影响 `ScriptParser`/`ScriptRunner` 的 `say` 指令处理流程 —— 需设计翻译 key 约定 (如 `@dialogue_001`)
- Qt `.qrc` 资源编译到二进制中会增大 exe 体积；外部资源包方案需要 `QResource::registerResource()`
- CMake 已支持 MinGW，macOS/Linux 构建需验证 Clang/GCC 兼容性
